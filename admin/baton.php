...
if(!isset($_GET['regiao'] )|| $regiao == 'Todos'){
    ...
    $consulta_baton = $conn->prepare("SELECT VENDEDOR, meta_baton, COUNT(NOME_parceiro) as realizado, if(meta_baton - COUNT(NOME_parceiro)<0,0,meta_baton - COUNT(NOME_parceiro)) as dif FROM (SELECT b.VENDEDOR, a.NOME_PARCEIRO, b.vendedor as vend, b.meta_baton FROM $mes a, $meta b where a.MATERIAL IN ($bat) AND a.QUANTIDADE>0 and a.vendedor = b.rca group by a.id)SUB GROUP BY VENDEDOR");
    $consulta_baton->execute();
    $result_baton = $consulta_baton->fetchAll();
             ...
             <ul class="nav">
                 ...
                 <li class="button-dropdown" ><a href="javascript:void(0)" class="dropdown-toggle">  Positivações  <span>▼</span>  </a>
                     <ul class="dropdown-menu">
                         ...
                         <li role="presentation" id="active" ><a href="#">Baton</a></li>
