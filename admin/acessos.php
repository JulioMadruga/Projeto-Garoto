...
date_default_timezone_set('America/Cuiaba'); 
 ...
 if(isset($_GET['regiao'])){
     ...
     $consulta_acessos= $conn->prepare("SELECT nome, nacesso, acesso FROM usuarios where regiao = '$regiao' order by nome");
...
if(!isset($_GET['regiao'] )|| $regiao == 'Todos'){
 ...
 $consulta_acessos= $conn->prepare("SELECT nome, nacesso, acesso FROM usuarios order by nome");
 $consulta_acessos->execute(array('id' => $id));
 $result_acessos= $consulta_acessos->fetchAll();
...
<td style="width: 150px; text-align: center; border: solid; border-color: #245269;">Nome</td>
<td style="width: 150px; text-align: center; border: solid; border-color: #245269;">Numero de Acessos</td>
<td style="width: 150px; text-align: center; border: solid; border-color: #245269;">Data do Último Acesso</td>
    ...
    foreach($result_acessos as $row) {
        ...
        extract($row);
                 ...
                 do {  
                           ...
                           echo'<td style="width: 150px; text-align: center; border: solid; border-color: #245269;">'.$row[0].'</td>';
                           echo '<td style="width: 150px; text-align: center; border: solid; border-color: #245269;">'.$row[1].'</td>';
                           echo '<td style="width: 150px; text-align: center; border: solid; border-color: #245269;">'.date('d/m/Y',strtotime($row[2])).'</td>';
