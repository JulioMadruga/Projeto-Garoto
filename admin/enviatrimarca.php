...
$consulta_metatri = $conn->prepare(" select a.vendedor, a.trimarca,a.rca, b.email, c.email, c.nome from $meta a, usuarios b, supervisor c where a.rca = b.rca and a.rca = c.rca order by vendedor");
...
//$mail->addAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
    ...
    $mail->Subject = 'ACOMPANHAMETO BIMARCA';
        ...
        $consulta_tri= $conn->prepare("SELECT Cod_Cliente,Nome,rca from clientes WHERE Cod_Cliente NOT IN (select id from(Select vendedor,nome_parceiro, sum(baton)as baton, sum(talento) as talento, trimarca, rca, id from
 ...
 and a.quantidade>0 and a.vendedor= b.rca) sub group by nome_parceiro)sub where baton>0 and talento>0 and rca = '$row[2]')and rca = '$row[2]'");
