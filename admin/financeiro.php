...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Importação Cobrança</title>
   ...
   <link rel="stylesheet" href="../css/bootstrap-theme.css">
    ...
    <body>
                  ...
                  <ul class="nav">
                      ...
                      <li class="button-dropdown"><a href="javascript:void(0)" class="dropdown-toggle">Importacao<span>▼</span>  </a>
                          <ul class="dropdown-menu">
                              ...
                              <li role="presentation" id="active"><a href="#">Financeiro</a></li>
        ...
        <div class="col-md-3"></div>  
            <div class="col-md-6" style="height: 70px; text-align:center; background-color:#1B6D85; font-family:Oswald; color:#F2FAFD;"> <h2>IMPORTAÇÃO DAS COBRANÇA</h2></div>
               <div class="col-md-3"></div>  
        </div>
...
if (isset($_POST['submit'])) {
    ...
    If ($verific == 7) {
        ...
        if ($escolha == 1) {
            ...
            $deleterecords = "TRUNCATE TABLE financeiro";//Esvaziar a tabela
            ...
            while (($dados = fgetcsv($handle, 1000, ";")) !== FALSE) {
                $dados = array_map("utf8_encode", $dados);
                  ...
                  $import= $conn->prepare("INSERT INTO financeiro (Cod_Cli,NF, Data_Fat, Cond_Pag, Venc, Dt_Atraso, Valor) Values ('$dados[0]','$dados[1]','$dados[2]','$dados[3]','$dados[4]','$dados[5]','$dados[6]')");
                 var_dump($import);
                   $import->execute();
            ...
            $deleterecords = "TRUNCATE TABLE bloqueados";//Esvaziar a tabela
            $limpa = $conn->prepare($deleterecords);
            $limpa->execute();
            ...
            while (($dados = fgetcsv($handle, 1000, ";")) !== FALSE) {
                $dados = array_map("utf8_encode", $dados);
                  ...
                  $import= $conn->prepare("INSERT INTO bloqueados (id,razao,ordem,cidade,uf,data_fat,valor,motivo) Values ('$dados[0]','$dados[1]','$dados[2]','$dados[3]','$dados[4]','$dados[5]','$dados[6]','')");
                   var_dump($import);
                $import->execute();
...
echo '<input type="button" value="Voltar" onClick="history.go(-1)"> ';
 echo "</br>";
  ...
  //  print "Transferir novos arquivos CSV selecionando o arquivo e clicando no botão Upload<br />\n";
    ...
    print '<input  type="radio" name="opcao" value="1" required> Inadimplencia ';
    print '<input  type="radio" name="opcao" value="2" style="margin-left: 30px">broqueados<br>';
    
       
    print "<input type='submit' name='submit' value='Importar Arquivo' style='margin-top: 15px' class='btn-lg btn-default'></form>";
